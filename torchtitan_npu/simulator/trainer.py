...
from torchtitan_npu.simulator.hardware_shims.block_diffusion_attention import (
    apply_block_diffusion_attention_shim,
)
...
class SimulationTrainer(Trainer):
    ...
    def __init__(self, config: SimulationTrainerConfig) -> None:
        ...
        if (
            config.simulation.selective_ac_save_ops is not None
            and config.activation_checkpoint.mode != "selective"
        ):
            raise ValueError(
                "simulation.selective_ac_save_ops requires "
                "activation_checkpoint.mode=selective"
            )
        with selective_ac_save_ops_context(
