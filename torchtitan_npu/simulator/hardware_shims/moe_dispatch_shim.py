...
def _record_fp8_payload_and_scale(
    ...
    *,
    ...
    """Record paired FP8 payload/E8M0-scale A2As with payload outermost."""
...
class _SimFP8DispatchAllToAll(torch.autograd.Function):
    """Model FP8 dispatch transport while preserving BF16 compute tensors.

    The downstream meta graph remains BF16 because the production direct-FP8
    grouped-MM autograd bridge is not installed yet. Communication records
    send payload then scale in forward and reverse that order in backward, so
    the FP8 payload A2A remains outermost.
    """
...
def run_meta_fp8_dispatch_all_to_all(
    ...
    """Model FP8 payload/scale dispatch with BF16 compute tensors."""
