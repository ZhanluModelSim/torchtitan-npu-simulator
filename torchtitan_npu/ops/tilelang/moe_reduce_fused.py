...
# Original file: examples/tile_kernels/moe/moe_reduce_fused.py
...
def get_reduce_fused_generic_kernel(
    hidden: int,
    num_topk: int,
    ...
    def reduce_fused_generic_kernel(
        ...
        topk_weights_flat: T.Tensor[(num_tokens * num_topk,), TL_FLOAT32],
        token_topk_to_pos_flat: T.Tensor[(num_tokens * num_topk,), TL_INT32],
            ...
            for iter_idx in T.serial(num_iters):
                ...
                if block_id < num_token_blocks:
                    ...
                    if token_base < num_tokens:
                        ...
                        T.copy(
                            token_topk_to_pos_flat[metadata_offset : metadata_offset + metadata_count],
                            topk_to_pos_rows_ub[0:metadata_count],
                            pad_value=-1,
                        )
                        if with_weights:
                            T.copy(
                                topk_weights_flat[metadata_offset : metadata_offset + metadata_count],
                                topk_weights_rows_ub[0:metadata_count],
                                pad_value=0.0,
                            )
...
def get_reduce_fused_specialized_kernel(
    hidden: int,
    num_topk: int,
    in_dtype: torch.dtype,
    out_dtype: torch.dtype,
        ...
        with T.Kernel(num_cores, is_npu=True) as (cid, vid), T.Scope("V"):
            ...
            if block_id < num_token_blocks:
                ...
                if token_base < num_tokens:
                    ...
                    T.copy(
                        token_topk_to_pos_flat[metadata_offset : metadata_offset + metadata_count],
                        topk_to_pos_rows_ub[0, 0:metadata_count],
                        pad_value=-1,
                    )
                    if with_weights:
                        T.copy(
                            topk_weights_flat[metadata_offset : metadata_offset + metadata_count],
                            topk_weights_rows_ub[0, 0:metadata_count],
                            pad_value=0.0,
                        )
                    ...
                    if token_base < num_tokens:
                        ...
                        if next_block_id < num_token_blocks:
                            ...
                            if next_token_base < num_tokens:
                                ...
                                if metadata_stage == 0:
                                    T.copy(
                                        token_topk_to_pos_flat[
                                            next_metadata_offset : next_metadata_offset + metadata_count
                                        ],
                                        topk_to_pos_rows_ub[1, 0:metadata_count],
                                        pad_value=-1,
                                    )
                                    if with_weights:
                                        T.copy(
                                            ...
                                            topk_weights_rows_ub[1, 0:metadata_count],
                                            pad_value=0.0,
                                        )
