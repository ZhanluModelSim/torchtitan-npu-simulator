...
class HiF8TrainingWeightWrapperTensor(BaseTrainingWeightWrapperTensor):
    ...
    Intercepts computation ops via :meth:`__torch_function__` and performs a
    real per-tensor HiF8 (``torch_npu.hifloat8``) quantized matmul using
    ``to_hif8_then_mm`` / ``to_hif8_then_grouped_mm``. Data stays in
    low-precision throughout the matmul, in contrast to fake-quantize wrappers
    which quantize then dequantize back to high precision before the matmul.

    ``weight_config`` and ``activation_config`` must be set and equal (the
    underlying kernel always quantizes both operands to the same dtype).
    """
    ...
    def _hif8_grouped_mm(cls, args, kwargs):
        ...
        with torch._C.DisableTorchFunctionSubclass():
            return to_hif8_then_grouped_mm(A, B_data, group_list, B.activation_config, B.weight_config)  # type: ignore
