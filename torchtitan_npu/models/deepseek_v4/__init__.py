...
def _register_step_pre_hooks(optimizers, model_parts, parallel_dims) -> None:
    ...
    ``LoggedAuxLoss`` snapshots its accumulators from the optimizer pre-hook,
    so both model-level hooks are installed through the same callback.
    """
...
def _make_v4_attn_config(
    *,
    ...
    inner_attention_cfg = DSAFlexAttention.Config(
        ...
        compress_ratio=compress_ratio,
        softmax_scale=softmax_scale,
        index_selection=index_selection_cfg,
