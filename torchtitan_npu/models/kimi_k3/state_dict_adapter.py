...
class KimiK3StateDictAdapter(StateDictAdapter):
    ...
    def __init__(self, model_config, hf_assets_path: str | None = None):
        ...
        self.from_hf_layer_map = {
            ...
            # MLA-specific
            "model.layers.{}.self_attn.q_a_proj.weight": "layers.{}.attention.q_a_proj.weight",
            "model.layers.{}.self_attn.q_a_layernorm.weight": "layers.{}.attention.q_a_layernorm.weight",
            "model.layers.{}.self_attn.q_b_proj.weight": "layers.{}.attention.q_b_proj.weight",
            ...
            "model.layers.{}.self_attn.g_proj.weight": "layers.{}.attention.g_proj.weight",
            ...
            "model.layers.{}.self_attn.q_conv1d.conv.weight": "layers.{}.attention.q_conv1d.conv.weight",
            "model.layers.{}.self_attn.k_conv1d.conv.weight": "layers.{}.attention.k_conv1d.conv.weight",
            "model.layers.{}.self_attn.v_conv1d.conv.weight": "layers.{}.attention.v_conv1d.conv.weight",
            ...
            # Dense FFN
            "model.layers.{}.mlp.gate_proj.weight": "layers.{}.feed_forward.gate_proj.weight",
            "model.layers.{}.mlp.up_proj.weight": "layers.{}.feed_forward.up_proj.weight",
            "model.layers.{}.mlp.down_proj.weight": "layers.{}.feed_forward.down_proj.weight",
            # MoE
            "model.layers.{}.mlp.gate.weight": "layers.{}.moe.gate.weight",
            "model.layers.{}.mlp.gate.e_score_correction_bias": "layers.{}.moe.gate.e_score_correction_bias",
            "model.layers.{}.mlp.routed_expert_down_proj.weight": "layers.{}.moe.routed_expert_down_proj.weight",
            "model.layers.{}.mlp.routed_expert_up_proj.weight": "layers.{}.moe.routed_expert_up_proj.weight",
            "model.layers.{}.mlp.routed_expert_norm.weight": "layers.{}.moe.routed_expert_norm.weight",
            "model.layers.{}.mlp.shared_experts.gate_proj.weight": "layers.{}.moe.shared_experts.gate_proj.weight",
            "model.layers.{}.mlp.shared_experts.up_proj.weight": "layers.{}.moe.shared_experts.up_proj.weight",
            "model.layers.{}.mlp.shared_experts.down_proj.weight": "layers.{}.moe.shared_experts.down_proj.weight",
        }
    ...
    def from_hf(self, hf_state_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        for hf_key, tensor in hf_state_dict.items():
            ...
            # Expert weights: model.layers.{i}.mlp.experts.{j}.w1.weight etc.
            if not mapped and "mlp.experts" in hf_key:
                expert_match = re.match(
                    r"model\.layers\.(\d+)\.mlp\.experts\.(\d+)\.(w[123])\.weight$", hf_key
                )
                if expert_match:
                    layer_idx, expert_idx, weight_name = expert_match.groups()
                    titan_key = f"layers.{layer_idx}.moe.experts.{expert_idx}.{weight_name}.weight"
                    state_dict[titan_key] = tensor
                    mapped = True
    ...
    def to_hf(self, state_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in state_dict.items():
            ...
            if not mapped:
                expert_match = re.match(
                    r"layers\.(\d+)\.moe\.experts\.(\d+)\.(w[123])\.weight$", key
                )
                if expert_match:
                    layer_idx, expert_idx, weight_name = expert_match.groups()
                    hf_key = f"model.layers.{layer_idx}.mlp.experts.{expert_idx}.{weight_name}.weight"
                    hf_state_dict[hf_key] = value
                    mapped = True
