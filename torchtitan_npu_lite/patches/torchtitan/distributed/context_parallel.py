...
from torchtitan_npu_lite.patches.torchtitan.distributed.varlen_cp import (
    CPVarlenMetadata,
)
...
def patched_cp_shard(
    ...
    """Objective: handle ``VarlenMetadata`` under CP via ``CPVarlenMetadata.from_global``.

    The upstream ``cp_shard`` does not know about ``VarlenMetadata``.
    When the mask is varlen, we shard Q as usual but construct per-rank
    ``CPVarlenMetadata`` instead of sharding the mask itself.
    """
