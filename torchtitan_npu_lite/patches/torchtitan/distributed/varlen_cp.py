...
Houses :class:`CPVarlenMetadata`, the CP-specific variant of
...
Mirrors ``torchtitan/distributed/varlen_cp.py`` from upstream PR #3430.
"""
...
class CPVarlenMetadata:
    """Per-rank varlen metadata for context-parallel varlen attention.

    Standalone frozen dataclass (not inheriting from ``NamedTuple``
    :class:`VarlenMetadata`) to avoid subclassing conflicts before upstream
    PR #3430 converts ``VarlenMetadata`` to a frozen dataclass.  Follows the
    same field layout and ``from_global`` interface so migration is seamless.

    ``k_global_gather_indices`` is a required 1-D gather index of length
    ``cu_seq_k[-1]``.  :meth:`VarlenAttention.forward` detects this via
    ``getattr(attention_masks, "k_global_gather_indices")`` and applies
    ``index_select(0, k_global_gather_indices)`` to K (and V) before the
    kernel call; the base type and non-CP callers are unaffected.

    Because CP shards the sequence dim, after K and V are allgathered the
    rank-local packed K can have unused gaps between per-segment visible
    regions when ``B > 1``; ``k_global_gather_indices`` picks out just those
    regions.  When a load balancer is active the indices also compose with
    the per-batch inverse permutation so the gather hits the correct entries
    in the rearranged K/V.
    """
    ...
    def from_global(
        ...
        whose packed form is ``global_metadata``; they are used to decompose
        the rank's shard per-batch and must satisfy
        ...
        segment with ``seqlen_k`` = (global doc-relative offset at the chunk
        end) + 1, so FA's right-aligned causal reproduces document-causal
        masking exactly.
        ...
        are fused into the single ``torch.stack(...).cpu().tolist()`` below;
        avoid adding more.

        Example (segment layout, ``B = 1``)::

            seq_len=20, B=1, 3 docs (cu_seq_q=[0,7,13,20]), CP=4, no load
            balancer. Rank 2 owns Q rows 10..14 which span the tail of doc 1
            and the head of doc 2, giving two segments::
        ...
        Example (non-contiguous K under ``B > 1``)::
            ...
            K_packed (length B*S = 40) places batch 1 after batch 0, so a
            rank's visible K per segment lives inside one batch's range;
            consecutive segments may belong to different batches.
            k_global_gather_indices (length 16) concatenates the visible
            slices [7..13), [13..15), [27..33), [33..35).
        """
        ...
        # Per-batch -> packed global positions, row-major across batch (matches
        ...
        # Fuse max_q / max_k / total_k into a single D2H transfer to keep only one
        # D2H.
        max_q, max_k, total_k = (
