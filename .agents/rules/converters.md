...
所有算子转换必须通过 `torchtitan_npu/converters/registry.py` 的 `@register_model_converter()` 注册。不要在模型文件中硬编码算子替换逻辑。
...
- `registry.py` — 转换器注册表（`@register_model_converter()`、`has_npu_converter()` 等）；**仅包含纯注册表功能，不执行 monkey-patch**
- `framework/` — 基于上游 `ModelConverter` 接口的通用包装、注册表实现和 state_dict/parallelize plan 更新逻辑
...
- 新增或重写 kernel/converter 时，在模块注释或类注释中说明：替换的原始算子、替换原因（性能/精度/兼容性）、NPU 硬件要求。
- 涉及数值输出的新增/替换 kernel 必须附带精度对比数据或引用已有的精度验证结果。
