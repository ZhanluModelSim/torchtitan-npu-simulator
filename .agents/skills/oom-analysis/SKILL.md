...
- OOM 错误的完整训练日志（必需）。
- 模型模块（`MODULE` 或 `--module`）、config 注册名（`CONFIG` 或 `--config`）和 CLI 覆盖参数（必需）。
- 对应模型的 `torchtitan_npu/models/<model>/config_registry.py` 位置（必需）。
...
**训练参数**：从 `torchtitan_npu/models/<model>/config_registry.py` 的 config 注册函数和 CLI 覆盖参数获取 `local_batch_size`、`seq_len`、TP/PP/DP/EP/CP 各并行度、`activation_checkpoint` 策略、优化器类型。
...
```bash
--training.torch_npu_memory_ratio 0.9  # 限制 PTA 最多使用 90% 显存，留 10% 给 workspace/HCCL
```
...
--profiling.profile_freq 5  # 每 5 步采集一次（首次分析建议用较小值）
```
...
| 优化器状态占比 | > 40% | 启用 `--optimizer.swap-optimizer` 或 FSDP 分片 |
...
| 问题诊断 | 优化措施 | CLI 覆盖参数或 `config_registry.py` 配置修改 | 预期效果 |
|---------|---------|-------------|---------|
...
| 激活内存过大 | 减小 batch_size | `--training.local_batch_size <较小值>` | 激活内存近似线性降低 |
| 激活内存过大 | 减小 seq_len | `--training.seq_len <较小值>` | 激活内存近似线性降低 |
| 优化器状态过大 | 启用 swap optimizer | `--optimizer.swap-optimizer --optimizer.swap-optimizer-times 16` | 优化器状态卸载到 CPU |
| 参数+梯度内存过大 | 增大 TP 并行度 | `--parallelism.tensor_parallel_degree <更大值>` | 参数/梯度按 TP 切分 |
| 参数+梯度内存过大 | 增大 PP 并行度 | `--parallelism.pipeline_parallel_degree <更大值>` | 参数按层切分到不同卡 |
| MoE Expert 内存过大 | 增大 EP 并行度 | `--parallelism.expert_parallel_degree <更大值>` | Expert 在更多卡间分布 |
| 内存碎片化严重 | 清理缓存 | 在代码中适时调用 `torch.npu.empty_cache()` | 释放碎片化内存 |
| 长序列内存过大 | 启用 Context Parallel | `--parallelism.context_parallel_degree <更大值>` | 序列维度切分 |
...
- 具体优化建议（含 `config_registry.py` 配置修改或 CLI 覆盖参数示例）
- 优化后的预期内存占用
- 是否需要进一步分析的建议
