...
层内和跨 action 的 prefetch 都直接使用来源模块入口的真实前驱：来源层输入/梯度 readiness
与本层参数 all-gather 同时连接到来源层 compute 和目标层 all-gather，二者从同一入口并行，
目标层 compute 再等待目标层 all-gather。导出的 L1 图不包含 FSDP 零开销控制算子。这样既能
阻止 all-gather 链在尚未到达来源层 hook 时继续向后预取，也不会把目标 all-gather 错误推迟到
来源层 attention 或整层计算完成之后。RS 只等待对应参数梯度生产者，HSDP AR 只等待对应 RS；
不同参数组的 RS/AR 不因捕获顺序而互相串行。即使后端暂时把 DualPipeV 的双图 overlap
串行化，也必须保留单张 L1 图内部的这些依赖与通信/计算并行关系。
