...
> 历史说明：本文记录最初以 `pipeline_order_with_comms` 为结构源的方案。
> 当前实现已经改为以实际语义抓取流为主源，lowered plan 只作兼容输入；
> `schedule_order` 也来自 rank-local 语义 action 顺序，而不是 L0 `seq_idx`。
> 当前契约见
> [PP L2 Capture Architecture](./pp-l2-capture-architecture.md)。
...
`PipelineScheduleSingle` 不走 `_perform_action`、无 `pipeline_order_with_comms`，需要由 trace assembler 从 compute span、P2P、FSDP residency 和 collective 记录合成 plan。当前仅按 timeline/CommEvent 排序的 fallback 不完备，不能把 reduce-scatter 映射成 RESHARD，也不能用普通 `SEND -> RECV` DataSlot 阻止 RECV 提前发布。完整的数据契约、组装顺序、通信 rendezvous 和 DES 验收要求见 [`1f1b-schedule-plan-assembly-guide.md`](./1f1b-schedule-plan-assembly-guide.md)。

已经拿到 `SchedulePlan` 的上层消费者不需要重新理解捕获过程。权威依赖重建契约见
[`schedule-plan-dependency-reconstruction-contract.md`](./schedule-plan-dependency-reconstruction-contract.md)，
DES 状态机和内存生命周期的扩展说明见
...
3. **元数据推理阶段的伪 CommEvent**：DYNAMIC 模式 `_prepare_forward_infra`/`_compute_outputs` 跑的 unshard 会经 `_record_comm_with_l0` 记 CommEvent，但 L0 算子被 `_in_metadata_inference` 跳过，导致 CommEvent 的 `op_id` 指向上一条已记的 aten 算子（陈旧）。修复：`build_schedule_plan` 收集 all-gather 时过滤掉 `op_id` 解析不到 `comm.*` 算子的 CommEvent。

`ScheduleAction` 增 `comm_op_id: int`（该 action 的 L0 comm 算子 id）与 `is_noop: bool`（FSDP mesh=1 等 action 无实际状态变化时为 True）。UNSHARD 对应 all-gather；PyTorch lowered schedule 中的 RESHARD 仅释放 full parameter，不对应 reduce-scatter。

### 仿真回放入口

给定一个 UNSHARD 或 SEND/RECV action，取其 L0 算子详细信息：
...
- `action.comm_op_id != 0`：`find_op_node` 返回 `comm.allgather` 等 OpNode，含 shape/comm_bytes/flops。DataSlot（`action.produces`/`consumes` 引用）另带 `shape`/`dtype`/`volume_bytes`/`src_exit_op`/`dst_entry_op`，可串到 producer/consumer 的 L1 模板边界算子。
- 真实 RESHARD 的 `comm_op_id == 0` 且 `is_noop == False`：它消费前序 compute 产出的零字节 `control` slot，表示 full parameter 可以释放，不回放通信。

### 回放契约与 no-op 规则

- `ScheduleAction` 是 DES 的执行主体，`schedule_order` 表达 lowered plan 的控制顺序；`OVERLAP_F_B` 父 action 表达并发组，真正执行的是其子 action。
- `DataSlot` 是数据就绪依赖，不是独立操作。内部 slot 必须有一个可解析的 producer，且 action 侧 `consumes`/`produces` 必须与 slot 的 producer/consumer 互为索引。
- `CommDetail` 与 `comm_op_id` 描述 action 如何通信及其代价；仅有 producer/consumer 边不能还原 peer、group、primitive 和通信量。
- FSDP mesh size 为 1 或 stage 未使用 FSDP 时，保留 UNSHARD/RESHARD action 并标记 `is_noop=True`，但不创建 DataSlot，也不向任何 action 添加 `consumes`/`produces`。DES 对该 action 立即完成。
- 使用 FSDP 且 shard degree 大于 1 时，UNSHARD 没有匹配到真实 all-gather CommEvent 属于捕获不完整，构图阶段直接失败，不能降级成 no-op。
- 真实 RESHARD 必须依赖同 stage、plan 中位于其前面的最近一次 F/B/I/W compute；查找必须包含 `OVERLAP_F_B.sub_actions`。无法找到 producer 时直接失败，禁止导出悬空 slot。

当前 P0 用零字节 `control` slot 表达“compute 完成后才能 reshard”的就绪条件。要完整刻画参数张量生命周期，后续应把 `UNSHARD -> param_full -> COMPUTE`、RESHARD 的释放状态转换与 `param_shard -> next iteration/OPTIMIZER` 分开表达，不能仅凭控制 slot 推断参数的输入和输出状态。
...
- DualPipeV（dp_shard=2，真实 FSDP）：UNSHARD 分别绑定 all-gather，RESHARD 分别消费最后一个 B/W compute 产出的零字节 `control` slot，不绑定 reduce-scatter。
...
- 1F1B 的 MB1+ L0 图保持折叠：同一 `(stage, FSDP group, comp_type)` 的后续 UNSHARD 复用 MB0 all-gather 模板，但 alloc/free action 与 residency interval 仍逐 microbatch 保留。元数据推理产生且没有合法 stage/mb 的 residency 不进入训练 plan。
