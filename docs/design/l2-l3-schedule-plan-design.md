...
已经拿到 `SchedulePlan` 的上层消费者不需要重新理解捕获过程。依赖重建、action 状态机、跨 rank matcher 和内存生命周期的消费规则见 [`schedule-plan-des-consumer-guide.md`](./schedule-plan-des-consumer-guide.md)。
